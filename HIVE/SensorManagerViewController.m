...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        discoveredSensors = [[NSMutableArray arrayWithCapacity:10] init];
        isSearching = FALSE;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
	{
        cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellId] init];
    }
    ...
    if ( tableView == pairedTable )
    {
        if (editingStyle == UITableViewCellEditingStyleDelete)
        {
            ...
            if ( [settings removeDeviceParams:dp forSensorType:sensorType] )
            {
                ...
                deviceParams = [[settings deviceParamsForSensorType:sensorType] init];
...
- (void)configForSensorType:(WFSensorType_t)eSensorType onNetworks:(USHORT)usNetworks
{
	...
	deviceParams = [[[WFHardwareConnector sharedConnector].settings deviceParamsForSensorType:sensorType] init];
    isSearching = FALSE;
